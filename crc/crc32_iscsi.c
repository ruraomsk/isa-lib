...
Библиотека функций быстрого доступа к данным для архитектуры «Эльбрус» («ЭЛЬФ»)
    Функция вычисления контрольной суммы по стандарту ISCSI
uint32_t crc32_iscsi ( unsigned char *buf, int len, unsigned int init_crc,);
где:
...
нужного типа по стандарту ISCSI с использованием таблицы переходов. 
